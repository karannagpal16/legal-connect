...
const levels = [
  { level: 1, title: "Legal Rookie", xpRequired: 0, color: "text-white/60", accent: "#ffffff60", bg: "bg-white/10" },
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-3xl font-serif font-bold text-white">Your Legal Journey</h1>
        <p className="text-white/40 mt-1">Keep grinding. Every quest brings you closer to the courtroom.</p>
      </motion.div>
        ...
        <div className="relative z-10">
          <div className="flex items-start gap-5 mb-6">
            ...
            <div className="relative flex-shrink-0">
              <div className={`w-20 h-20 rounded-2xl ${currentLevel.bg} border border-white/15 flex flex-col items-center justify-center shadow-2xl`}
                ...
                <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider">LVL</span>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.label} className="bg-white/5 rounded-xl px-3 py-3 text-center border border-white/8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] text-white/30 bg-white/5 px-2 py-1 rounded-full border border-white/10">Resets daily</span>
        </div>
        <div className="space-y-2.5">
          {dailyChallenges.map((c, i) => (
            ...
            >
              ...
              <span className="text-white text-sm font-medium flex-1">{c.title}</span>
            ...
            <div
              ...
              className={`flex items-center gap-4 rounded-2xl px-4 py-3.5 border transition-all ${
                p.isYou
                  ...
                  : "bg-card/30 border-white/8"
              }`}
            >
              <span className="text-xl w-8 text-center">{p.badge}</span>
              <div className="flex-1">
                <span className={`text-sm font-bold ${p.isYou ? "text-emerald-400" : "text-white"}`}>{p.name}</span>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
        <h2 className="text-xs font-bold uppercase tracking-widest text-white/30 mb-4">All Features</h2>
          ...
          {navFeatures.map((f, i) => (
            <Link key={f.title} href={f.href}>
              ...
              >
                ...
                <p className="text-white text-xs font-bold mb-0.5">{f.title}</p>
