  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#1A2E2A]">Client Messages</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-[600px]">
        {/* Conversation list */}
        <div className="lg:col-span-1 bg-card/40 border border-[#1A2E2A]/10 rounded-2xl overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-[#1A2E2A]/8 bg-[#1A2E2A]/5">
            <p className="text-[#1A2E2A]/40 text-xs font-semibold uppercase tracking-wider">Conversations ({conversations.length})</p>
          </div>
            ...
            {conversations.map(conv => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-4 hover:bg-[#1A2E2A]/5 transition-all ${activeId === conv.id ? "bg-amber-500/10 border-l-2 border-amber-400" : ""}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="text-[#1A2E2A] text-sm font-semibold truncate">{conv.clientName}</p>
                        <span className="text-[#1A2E2A]/25 text-[10px] flex-shrink-0 ml-1">{timeAgo(conv.lastActivity)}</span>
                      </div>
                      <p className="text-[#1A2E2A]/35 text-xs truncate mt-0.5">{conv.caseTitle}</p>
                      {lastMsg && (
                        <p className="text-[#1A2E2A]/25 text-xs truncate mt-1">
        ...
        {/* Chat window */}
        <div className="lg:col-span-2 bg-card/40 border border-[#1A2E2A]/10 rounded-2xl flex flex-col overflow-hidden">
          {activeConv ? (
            <>
              {/* Header */}
              <div className="px-5 py-4 border-b border-[#1A2E2A]/8 bg-[#1A2E2A]/5 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-[#1A2E2A] font-bold text-sm">{activeConv.clientName}</p>
                  <p className="text-[#1A2E2A]/35 text-xs">{activeConv.caseTitle}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                {activeConv.messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isAdv ? "bg-amber-500/20 border border-amber-500/30" : "bg-[#1A2E2A]/10 border border-white/15"}`}>
                        <User className={`w-3.5 h-3.5 ${isAdv ? "text-amber-400" : "text-[#1A2E2A]/50"}`} />
                      </div>
                      <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${isAdv ? "bg-amber-500/15 border border-amber-500/20" : "bg-[#1A2E2A]/8 border border-[#1A2E2A]/10"}`}>
              ...
              <div className="px-4 py-4 border-t border-[#1A2E2A]/8">
                <div className="flex items-center gap-3 bg-[#1A2E2A]/5 border border-[#1A2E2A]/10 rounded-xl px-4 py-3 focus-within:border-amber-500/30">
                  <input
                    value={input}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={handleSend}
                      disabled={!input.trim()}
                      className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${input.trim() ? "bg-amber-500 text-background hover:bg-amber-600" : "bg-[#1A2E2A]/10 text-[#1A2E2A]/20"}`}
                    >
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 text-[#1A2E2A]/25">
              <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center">
                <MessageSquare className="w-8 h-8 text-amber-400/40" />
              </div>
              <div className="text-center">
                <p className="font-semibold text-[#1A2E2A]/40">Select a conversation</p>
