...
import { Bell, Calendar, Clock, Scale, Plus, Trash2, AlertCircle } from "lucide-react";
...
export function AdvocateReminders() {
  ...
  const [reminders, setReminders] = useState<ManualReminder[]>([
    ...
    },
    {
      ...
      note: "Prepare bail application docs",
      date: "2026-04-03",
      time: "03:00 PM",
      type: "Meeting",
    },
  ...
  const [form, setForm] = useState({
    ...
    date: "",
    time: "",
    type: "Hearing" as ManualReminder["type"],
  });
  ...
  const upcomingCases = cases
    .filter((c) => {
      ...
      try {
        return isAfter(parseISO(c.nextDate), new Date()) || isToday(parseISO(c.nextDate));
      } catch {
        return false;
      }
    })
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#1A2332]/40 ml-10">
            Upcoming hearings and deadlines for your practice.
          </p>
        </div>
      ...
      {showAdd && (
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <input
              placeholder="Note or case number"
              value={form.note}
            ...
            <select
              value={form.type}
              onChange={(e) =>
                setForm((f) => ({ ...f, type: e.target.value as ManualReminder["type"] }))
              }
              className="bg-[#1A2332]/5 border border-[#1A2332]/10 rounded-xl px-4 py-3 text-[#1A2332] text-sm focus:outline-none focus:border-amber-500/40"
            >
          ...
          <div className="flex gap-3">
            <button
              onClick={addReminder}
              className="bg-amber-500 hover:bg-amber-600 text-[#1A2332] px-5 py-2.5 rounded-xl text-sm font-bold transition-all"
            >
              Save
            </button>
            ...
            <button
              ...
              className="text-[#1A2332]/40 hover:text-[#1A2332] text-sm px-4 py-2.5 transition-all"
            >
              Cancel
            </button>
          </div>
              ...
              >
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-amber-400 text-sm font-bold">
                    {c.nextDate ? format(parseISO(c.nextDate), "MMM d") : "—"}
                  </p>
                  <p className="text-[#1A2332]/30 text-xs">
                    {c.nextDate && isToday(parseISO(c.nextDate)) ? "TODAY" : "Upcoming"}
                  </p>
                </div>
              </motion.div>
            ))}
      ...
      <div>
        ...
        <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  <span
                    className={`text-[11px] px-2 py-0.5 rounded-full border font-semibold ${typeColors[r.type]}`}
                  >
                    {r.type}
                  </span>

                  <span className="flex items-center gap-1 text-xs text-[#1A2332]/30">
          ...
          {reminders.length === 0 && (
            <div className="text-center py-8 text-[#1A2332]/30 text-sm">
              No manual reminders added yet.
            </div>
          )}
        </div>
      </div>
