...
const advocates: Advocate[] = [
  {
    id: 1, name: "Rishika Nagpal", title: "Senior Advocate",
    speciality: "Criminal & Constitutional Law", rating: 4.9, cases: 340,
    distance: "0.2 km", available: true, avatar: "RN", color: "bg-amber-500",
      ...
      <AnimatePresence mode="wait">
        ...
        {stage === "browse" && (
          <motion.div key="browse" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }}>
            <div className="mb-8">
              ...
              <p className="text-white/50">Available advocates near you — chat, consult, or call in seconds.</p>
            </div>
            ...
            <div className="relative mb-6">
              ...
              <input
                ...
                className="w-full bg-card/50 border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-white placeholder:text-white/30 focus:outline-none focus:border-primary/50 text-sm"
              />
            </div>
            ...
            <div className="space-y-4">
              {filtered.map((advocate, i) => (
                <motion.div
                  ...
                  className={`bg-card/40 border rounded-3xl overflow-hidden transition-all duration-300 ${advocate.available ? "border-white/10 hover:border-primary/30" : "border-white/5 opacity-60"}`}
                >
                  <div className="p-5 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-white/50 text-xs mb-1.5">{advocate.title} · {advocate.speciality}</p>
                      ...
                      {advocate.available ? (
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-bold transition-all border ${openChat === advocate.id ? "bg-primary/20 border-primary/40 text-primary" : "bg-white/5 hover:bg-white/10 border-white/10 text-white/60 hover:text-white"}`}
                          >
                    ...
                    {openChat === advocate.id && (
                      ...
                      >
                        <div className="border-t border-white/8">
                          <div className="px-4 py-2.5 flex items-center justify-between bg-white/3">
                            <div className="flex items-center gap-2">
                              <MessageSquare className="w-3.5 h-3.5 text-primary" />
                              <span className="text-white/60 text-xs font-semibold">Chat with {advocate.name.split(" ")[0]}</span>
                            ...
                            {(chatMessages[advocate.id] || []).map(msg => (
                              <div key={msg.id} className={`flex ${msg.from === "client" ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[80%] ${msg.from === "client" ? "order-1" : ""}`}>
                                  <div className={`rounded-2xl px-3.5 py-2.5 ${msg.from === "advocate" ? "bg-white/8 border border-white/10 rounded-tl-sm" : "bg-primary/15 border border-primary/20 rounded-tr-sm"}`}>
                                    <p className="text-white text-xs leading-relaxed">{msg.text}</p>
                                  </div>
                                  <p className={`text-white/15 text-[9px] mt-0.5 px-1 ${msg.from === "client" ? "text-right" : ""}`}>{timeStr(msg.ts)}</p>
                                </div>
                              </div>
                            ))}
                            <div ref={chatEndRef} />
                          ...
                          <div className="px-3 py-2.5 border-t border-white/8 flex gap-2">
                            <input
                              ...
                              className="flex-1 bg-white/5 border border-white/10 focus:border-primary/30 rounded-xl px-3 py-2 text-white text-xs placeholder:text-white/25 focus:outline-none transition-all"
                            />
                            <button
                              onClick={() => sendChat(advocate.id)}
                              disabled={!chatInput.trim()}
                              className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${chatInput.trim() ? "bg-primary hover:bg-primary/90 text-background" : "bg-white/8 text-white/20"}`}
                            >
        ...
        {stage === "matching" && selectedAdvocate && (
          ...
          >
            ...
            <p className="text-white/40 mb-2 text-sm">{selectedAdvocate.title} · {selectedAdvocate.speciality}</p>
            ...
            <div className="space-y-3 w-full max-w-xs">
              {[
                ...
                >
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center transition-all ${matchStep > i ? "bg-primary text-background" : "bg-white/10 text-white/20"}`}>
                    {matchStep > i ? (
            ...
            <div className="flex-1 relative overflow-hidden">
              ...
              <div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/50 text-xs">{selectedAdvocate.title} · {selectedAdvocate.fee.toLocaleString("en-IN")}/hr</p>
                </div>
                ...
                <div className="absolute bottom-28 right-4 w-28 h-36 bg-gray-800 rounded-2xl border border-white/20 overflow-hidden shadow-2xl flex items-center justify-center">
                  {videoOff ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-b from-gray-700 to-gray-900 flex items-center justify-center">
                      <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-sm">Me</div>
                    </div>
                  )}
                </div>
              )}
