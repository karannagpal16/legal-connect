    ...
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-3xl font-serif font-bold text-[#1A2332]">Law Made Simple</h1>
        </div>
        <p className="text-[#1A2332]/40 ml-10">Plain language guides for common legal situations. No jargon, just clarity.</p>
      </motion.div>
      ...
      <div className="space-y-4">
        {guides.map((guide, i) => (
          ...
          >
            ...
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 bg-[#1A2332]/10`}>
                <guide.icon className={`w-6 h-6 ${guide.color}`} />
              </div>
              <div className="flex-1">
                <h3 className="text-[#1A2332] font-bold text-base">{guide.title}</h3>
                <p className="text-[#1A2332]/40 text-xs mt-0.5 line-clamp-1">{guide.intro}</p>
              </div>
              {expandedId === guide.id ? (
            ...
            <AnimatePresence>
              {expandedId === guide.id && (
                <motion.div
                  ...
                  className="border-t border-[#1A2332]/10 bg-background/60"
                >
                  <div className="p-5">
                    <p className="text-[#1A2332]/70 text-sm mb-5 leading-relaxed">{guide.intro}</p>
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                            activeTab === tab
                              ...
                              : "bg-[#1A2332]/5 text-[#1A2332]/40 hover:text-[#1A2332]/60"
                          }`}
                        >
                      ...
                      <ol className="space-y-3">
                        {guide.steps.map((step, idx) => (
                          <li key={idx} className="flex items-start gap-3 text-sm">
                            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 bg-[#1A2332]/10 ${guide.color}`}>
                              {idx + 1}
                            </span>
                            <span className="text-[#1A2332]/70 leading-relaxed">{step}</span>
                          </li>
                        ))}
                      </ol>
                    )}
                      ...
                      <ul className="space-y-3">
                        {guide.rights.map((right, idx) => (
                          <li key={idx} className="flex items-start gap-3 text-sm">
                            ...
                            <span className="text-[#1A2332]/70 leading-relaxed">{right}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                    ...
                    <div className="mt-5 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                      ...
                      <p className="text-[#1A2332]/50 text-xs leading-relaxed">{guide.whenToHireAdvocate}</p>
                    </div>
                    ...
                    <div className="flex gap-3 mt-4">
                      ...
                      <Link href="/client/ai-assistant">
                        <button className="flex items-center gap-2 bg-[#1A2332]/5 hover:bg-[#1A2332]/10 text-[#1A2332]/60 border border-[#1A2332]/10 px-4 py-2 rounded-xl text-sm font-bold transition-all">
                          Ask AI
                        </button>
                      </Link>
                    </div>
