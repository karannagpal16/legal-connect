...
const initialReminders: Reminder[] = [
  ...
  {
    id: 3,
    caseTitle: "Defamation Case — Nagpal v Singh",
...
const statusBadge: Record<string, string> = {
  ...
  past: "text-white/30 bg-white/5 border border-white/10",
};
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 ml-10">Stay ahead of every hearing and deadline.</p>
        </div>
        <button
      ...
      {showAdd && (
        ...
        >
          <h3 className="font-semibold text-white mb-2">New Reminder</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input
              placeholder="Case Title"
              value={form.caseTitle}
              onChange={e => setForm(f => ({ ...f, caseTitle: e.target.value }))}
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-amber-500/40"
            />
            <input
            ...
            <select
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500/40"
            >
          ...
          <div className="flex gap-3">
            <button onClick={addReminder} className="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all">Save Reminder</button>
      ...
      <div>
        ...
        <div className="space-y-3">
          {upcoming.length === 0 && (
            <div className="text-center py-10 text-white/30 text-sm">No upcoming reminders</div>
          )}
          {upcoming.map((r, i) => (
            <motion.div
              key={r.id}
              ...
              className="bg-card/40 border border-white/10 rounded-2xl p-5 flex items-start gap-4 group hover:border-amber-500/20 transition-all"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <h3 className="font-semibold text-white text-sm leading-tight">{r.caseTitle}</h3>
                ...
                <p className="text-white/40 text-xs mb-2">{r.caseNumber} · {r.court}</p>
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 transition-opacity text-white/20 hover:text-rose-400 p-1"
              >
            ...
            {past.map((r, i) => (
              <div key={r.id} className="bg-card/20 border border-white/5 rounded-2xl p-5 flex items-start gap-4 opacity-60 group">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-white/60 text-sm">{r.caseTitle}</h3>
                  <p className="text-white/30 text-xs">{r.caseNumber} · {r.court}</p>
                  <p className="text-white/20 text-xs mt-1">{r.hearingDate} · {r.type}</p>
                </div>
                <button onClick={() => deleteReminder(r.id)} className="opacity-0 group-hover:opacity-100 text-white/20 hover:text-rose-400 p-1">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            ))}
