...
const ADVOCATES: Advocate[] = [
  {
    id: "adv-1", name: "Adv. Rishika Nagpal", title: "Senior Advocate", initials: "RN",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-serif font-bold text-white">Connect with an Advocate</h1>
            <p className="text-white/40 text-xs mt-0.5">Start with AI · Get matched · Chat securely</p>
          </div>
        </div>
          ...
          {(["chat", "select", "connected"] as Phase[]).map((p, i) => {
            ...
            return (
              <div key={p} className="flex items-center gap-1.5">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center border text-[10px] font-bold transition-all ${done ? "bg-emerald-500 border-emerald-500 text-white" : active ? "bg-primary border-primary text-background" : "bg-white/5 border-white/15 text-white/30"}`}>
                  {done ? <Check className="w-3.5 h-3.5" /> : <Icon className="w-3.5 h-3.5" />}
                </div>
                <span className={`text-xs font-semibold hidden sm:block ${active ? "text-primary" : done ? "text-emerald-400" : "text-white/25"}`}>{labels[i]}</span>
                {i < 2 && <ChevronRight className="w-3 h-3 text-white/15 hidden sm:block" />}
              </div>
            );
          })}
          {phase !== "chat" && (
            <button onClick={reset} className="ml-2 flex items-center gap-1 text-white/30 hover:text-white/60 text-xs transition-colors">
      ...
      <AnimatePresence mode="wait">
        {phase === "chat" && (
          <motion.div key="chat-phase" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="grid grid-cols-1 lg:grid-cols-4 gap-5">
            {/* Main chat window */}
            <div className="lg:col-span-3 bg-card/40 border border-white/10 rounded-2xl flex flex-col" style={{ height: "580px" }}>
              {/* Chat header */}
              <div className="px-5 py-4 border-b border-white/8 flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-white font-bold text-sm">LexBot — AI Assistant</p>
                  <p className="text-white/35 text-xs">Rishika Nagpal & Associates · Not a substitute for legal advice</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5">
                {messages.map((msg) => (
                  <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.role === "user" ? "flex-row-reverse" : "flex-row"} gap-3`}>
                    ...
                    <div className={`max-w-[80%] flex flex-col gap-1 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                      <div className={`rounded-2xl px-4 py-3 ${msg.role === "assistant" ? "bg-white/8 border border-white/10 rounded-tl-sm" : "bg-blue-500/15 border border-blue-500/20 rounded-tr-sm"}`}>
                      ...
                      <span className="text-white/20 text-[10px] px-1">{timeStr(msg.ts)}</span>
                      ...
                      {msg.suggestAdvocate && msg.id === messages[messages.length - 1]?.id && !streaming && (
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-2 bg-primary/10 border border-primary/25 rounded-xl p-3 w-full max-w-xs">
                          ...
                          <p className="text-white/50 text-xs mb-3">Choose from our available advocates for a private, encrypted consultation.</p>
                  ...
                  {showAdvocateSuggestion && !messages[messages.length - 1]?.suggestAdvocate && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-primary/10 border border-primary/25 rounded-2xl p-4 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-white/40 text-xs">For advice on your specific situation, speak to one of our advocates.</p>
                      </div>
                ...
                {showSuggestions && messages.length <= 1 && (
                  <div className="space-y-2">
                    <p className="text-white/25 text-xs font-semibold uppercase tracking-wider">Suggested questions</p>
                    <div className="flex flex-wrap gap-2">
                      {QUICK_SUGGESTIONS.map(q => (
                        <button key={q} onClick={() => { setInput(q); setShowSuggestions(false); sendToAI(q); }} className="text-xs bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/25 text-white/50 hover:text-white px-3 py-2 rounded-xl transition-all text-left">
                          {q}
                        </button>
                      ))}
                    </div>
              ...
              <div className="px-4 py-4 border-t border-white/8">
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 bg-white/5 border border-white/10 focus:border-primary/30 rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/25 focus:outline-none transition-all"
                  />
                  <button
                    onClick={handleSend}
                    disabled={!input.trim() || streaming}
                    className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${input.trim() && !streaming ? "bg-primary hover:bg-primary/90 text-background" : "bg-white/8 text-white/20"}`}
                  >
                    <Send className="w-4 h-4" />
                  </button>
                </div>
                <p className="text-center text-white/15 text-[10px] mt-2">LexBot provides general information only · Not legal advice</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-1 space-y-4">
              ...
              <div className="bg-primary/10 border border-primary/25 rounded-2xl p-5">
                ...
                <h3 className="text-white font-bold mb-1">Need real legal advice?</h3>
                <p className="text-white/40 text-xs leading-relaxed mb-4">LexBot handles general queries. For your specific situation, speak to one of our advocates.</p>
              ...
              {/* AI limitations */}
              <div className="bg-card/30 border border-white/8 rounded-2xl p-4 space-y-3">
                <p className="text-white/50 text-xs font-bold uppercase tracking-wider">What I can help with</p>
                {[
              ...
              <div className="bg-card/30 border border-white/8 rounded-2xl p-4">
                <p className="text-white/50 text-xs font-bold uppercase tracking-wider mb-3">Available Now</p>
                {ADVOCATES.filter(a => a.available).slice(0, 3).map(a => (
                  <div key={a.id} className="flex items-center gap-2 mb-2.5">
                    <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${a.color} flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0`}>{a.initials}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white/70 text-xs font-semibold truncate">{a.name}</p>
                      <p className="text-white/30 text-[10px] truncate">{a.specialisations[0]}</p>
                    </div>
        ...
        {phase === "select" && (
          <motion.div key="select-phase" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="space-y-5">
            <div className="bg-primary/8 border border-primary/20 rounded-2xl px-5 py-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-white text-sm font-bold">LexBot has summarised your inquiry</p>
                <p className="text-white/45 text-xs mt-0.5">Choose an advocate below — they'll receive a brief from LexBot so you don't have to repeat yourself.</p>
              </div>
            </div>

            <div>
              <h2 className="text-white font-bold text-lg mb-1">Choose Your Advocate</h2>
              <p className="text-white/40 text-sm">All consultations are private and end-to-end encrypted.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {ADVOCATES.map((adv, i) => (
                <motion.div
                  ...
                  className={`bg-card/40 border rounded-2xl p-5 flex flex-col gap-4 transition-all ${adv.available ? "border-white/10 hover:border-white/25" : "border-white/5 opacity-60"}`}
                >
                  {/* Advocate header */}
                  <div className="flex items-start gap-3">
                    <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${adv.color} flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-lg`}>
                      {adv.initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <p className="text-white font-bold text-sm truncate">{adv.name}</p>
                        {adv.available && <Circle className="w-2.5 h-2.5 text-emerald-400 fill-emerald-400 flex-shrink-0" />}
                      </div>
                      <p className="text-white/40 text-xs">{adv.title}</p>
                      <div className="flex items-center gap-1.5 mt-1">
                        <Stars rating={adv.rating} />
                        <span className="text-white/30 text-[10px]">{adv.rating} ({adv.reviews})</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {adv.specialisations.map(s => (
                      <span key={s} className="text-[10px] px-2 py-1 rounded-full bg-white/5 border border-white/10 text-white/50">{s}</span>
                    ))}
                  </div>
                  ...
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white/65 font-semibold">{adv.experience}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/35">Consultation</span>
                      <span className="text-amber-400 font-bold">₹{adv.fee.toLocaleString()}/hr</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/35">Availability</span>
                      <span className={`font-semibold ${adv.available ? "text-emerald-400" : "text-white/40"}`}>{adv.responseTime}</span>
                    </div>
                  </div>
                  ...
                  {adv.available ? (
                    <div className="space-y-2 mt-auto">
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          className={`flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-xl border transition-all ${selectedAdvocate?.id === adv.id && callType === "audio" ? "bg-blue-500 border-blue-500 text-white" : "bg-blue-500/10 border-blue-500/25 text-blue-400 hover:bg-blue-500/20"}`}
                          onClick={() => { setSelectedAdvocate(adv); setCallType("audio"); }}
                        >
                          <Phone className="w-3.5 h-3.5" /> Audio
                        </button>
                        <button
                          className={`flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-xl border transition-all ${selectedAdvocate?.id === adv.id && callType === "video" ? "bg-violet-500 border-violet-500 text-white" : "bg-violet-500/10 border-violet-500/25 text-violet-400 hover:bg-violet-500/20"}`}
                    ...
                    <div className="mt-auto">
                      <button className="w-full text-xs font-bold text-white/30 bg-white/5 border border-white/10 py-2.5 rounded-xl cursor-not-allowed">
            ...
            <div className="lg:col-span-1 space-y-4 order-2 lg:order-1">
              <div className="bg-card/40 border border-white/10 rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${selectedAdvocate.color} flex items-center justify-center text-white font-bold shadow-lg flex-shrink-0`}>
                    {selectedAdvocate.initials}
                  </div>
                  <div>
                    <p className="text-white font-bold text-sm">{selectedAdvocate.name}</p>
                    <p className="text-white/40 text-xs">{selectedAdvocate.title}</p>
                ...
                <div className="space-y-2 text-xs border-t border-white/8 pt-3">
                  <div className="flex justify-between">
                    <span className="text-white/35">Experience</span>
                    <span className="text-white/65">{selectedAdvocate.experience}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-white/35">Rating</span>
              ...
              <div className="bg-emerald-500/8 border border-emerald-500/20 rounded-2xl p-4">
                ...
                <p className="text-white/40 text-xs leading-relaxed">Only you and {selectedAdvocate.name} can read these messages. Encrypted with AES-256.</p>
              </div>

              <button onClick={() => setPhase("select")} className="w-full text-xs font-bold text-white/30 hover:text-white/60 bg-white/5 hover:bg-white/10 border border-white/10 py-2.5 rounded-xl transition-all flex items-center justify-center gap-1.5">
                <X className="w-3.5 h-3.5" /> Change Advocate
              </button>
            </div>

            {/* Chat window */}
            <div className="lg:col-span-3 bg-card/40 border border-white/10 rounded-2xl flex flex-col order-1 lg:order-2" style={{ height: "580px" }}>
              {/* Header */}
              <div className="px-5 py-4 border-b border-white/8 flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${selectedAdvocate.color} flex items-center justify-center text-white font-bold text-sm flex-shrink-0`}>
                  {selectedAdvocate.initials}
                </div>
                <div className="flex-1">
                  <p className="text-white font-bold text-sm">{selectedAdvocate.name}</p>
                  <p className="text-white/35 text-xs">{selectedAdvocate.specialisations.join(" · ")}</p>
                </div>
                <div className="flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
                  ...
                  return (
                    <motion.div key={msg.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`flex ${isClient ? "flex-row-reverse" : "flex-row"} gap-2.5`}>
                      <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center border text-[10px] font-bold ${isClient ? "bg-blue-500/20 border-blue-500/30 text-blue-400" : `bg-gradient-to-br ${selectedAdvocate.color}`}`}>
                        {isClient ? <User className="w-4 h-4 text-blue-400" /> : <span className="text-white">{selectedAdvocate.initials[0]}</span>}
                      </div>
                      <div className={`max-w-[78%] rounded-2xl px-4 py-3 ${isClient ? "bg-blue-500/15 border border-blue-500/20 rounded-tr-sm" : "bg-white/8 border border-white/10 rounded-tl-sm"}`}>
                        {!isClient && <p className="text-amber-400 text-[10px] font-bold mb-1">{selectedAdvocate.name}</p>}
                        <p className="text-white text-sm leading-relaxed">{xorDecrypt(msg.text)}</p>
                        <div className="flex items-center justify-end gap-1 mt-1">
                          <Lock className="w-2.5 h-2.5 text-white/20" />
                          <span className="text-white/20 text-[10px]">{timeStr(msg.ts)}</span>
                        </div>
                      </div>
                    </motion.div>
                  );
              ...
              <div className="px-4 py-4 border-t border-white/8">
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${connectedInput.trim() ? "bg-primary hover:bg-primary/90 text-background" : "bg-white/8 text-white/20"}`}
                  >
                    <Send className="w-4 h-4" />
                  </button>
                </div>
