...
(function () {
  ...
  function render5StageStepperHtml(booking) {
    ...
    const stages = [
      { key: "booking_submitted", num: 1, title: "Submitted", sub: "Fee held in escrow" },
      { key: "acknowledged_and_assigned", num: 2, title: "Assigned", sub: "Legal Connect review" },
      ...
      <div class="lc-stepper-card" data-booking-id="${escapeHtml(booking.id || 'BK-9012')}">
        <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e2e8f0;padding-bottom:10px;">
          <div>
            ...
            <p style="margin:2px 0 0;font-size:0.8rem;color:#64748b;">Booking #${escapeHtml(booking.id || 'BK-9012')} · Work Completion Hold: <span style="color:#0f766e;font-weight:700;">${booking.workHoldStatus === 'released' ? 'Released' : 'Active Escrow'}</span></p>
          </div>
        ...
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:12px;display:grid;gap:6px;font-size:0.86rem;">
          <div><strong>Assigned Lawyer:</strong> ${escapeHtml(booking.assignedAdvocateName || "Adv. Rishika Nagpal (Delhi High Court Panel)")}</div>
          ${booking.caseNumber ? `<div><strong>Case / CNR No:</strong> ${escapeHtml(booking.caseNumber)}</div>` : ''}
          ${booking.courtName ? `<div><strong>Court Name:</strong> ${escapeHtml(booking.courtName)} (${escapeHtml(booking.caseType || 'General')})</div>` : ''}
          ${booking.problemSummary ? `<div><strong>Problem Summary:</strong> ${escapeHtml(booking.problemSummary)}</div>` : ''}
          ${booking.attachedFiles && booking.attachedFiles.length ? `
            <div style="margin-top:4px;">
              <strong>Uploaded Documents (${booking.attachedFiles.length}):</strong>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:flex-end;margin-top:4px;">
          ${isLawyerOrAdmin ? `
            ${currentIndex < 1 ? `<button type="button" class="gold-button" data-advance-stage="acknowledged_and_assigned" data-booking-target="${escapeHtml(booking.id)}">Acknowledge Request</button>` : ''}
            ${currentIndex < 2 ? `<button type="button" class="gold-button" data-advance-stage="advocate_connected" data-booking-target="${escapeHtml(booking.id)}">Connect as Advocate</button>` : ''}
      ...
      modal.innerHTML = `
        <div class="lc-intake-modal">
          <div class="lc-intake-header">
            <div>
              <h3>Transparent Booking & Case Intake</h3>
              <p style="margin:2px 0 0;font-size:0.8rem;color:#64748b;">Step ${currentStep} of 4 · ${
                currentStep === 1 ? 'Contact & Verification' :
                currentStep === 2 ? 'Case Intake Details' :
                currentStep === 3 ? 'Document Uploads' : 'Select Plan & Pay Fees'
              }</p>
            </div>
          ...
          ${currentStep === 2 ? `
            <div style="display:grid;gap:12px;">
              <label>Case / Matter Title <input type="text" id="intake-case-title" value="${escapeHtml(intakeData.caseTitle)}" placeholder="e.g. Property Title Dispute / Notice Reply / Bail Matter" /></label>
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                ...
                <label>Court Name <input type="text" id="intake-court" value="${escapeHtml(intakeData.courtName)}" placeholder="e.g. Tis Hazari Court / High Court" /></label>
              </div>
              <label>Case Category 
                <select id="intake-category">
                  <option value="Criminal" ${intakeData.caseType === 'Criminal' ? 'selected' : ''}>Criminal Law</option>
                  <option value="Civil" ${intakeData.caseType === 'Civil' ? 'selected' : ''}>Civil / Commercial</option>
                  <option value="Property" ${intakeData.caseType === 'Property' ? 'selected' : ''}>Property & Real Estate</option>
                  <option value="Family" ${intakeData.caseType === 'Family' ? 'selected' : ''}>Family & Matrimonial</option>
                  <option value="Consumer" ${intakeData.caseType === 'Consumer' ? 'selected' : ''}>Consumer Protection</option>
                  <option value="Labour" ${intakeData.caseType === 'Labour' ? 'selected' : ''}>Labour & Employment</option>
                </select>
              </label>
              <label>Problem Summary <textarea id="intake-summary" rows="3" placeholder="Describe what happened, what notices were received, and what help you need from counsel...">${escapeHtml(intakeData.problemSummary)}</textarea></label>
            </div>
          ` : ''}
          ...
          ${currentStep === 3 ? `
            <div style="display:grid;gap:12px;">
              <div style="background:#f8fafc;border:1px dashed #cbd5e1;border-radius:16px;padding:16px;text-align:center;">
                <p style="margin:0 0 8px;font-weight:700;">Upload Case Documents / Notices / Photos</p>
                <label style="margin:0 0 8px;">Document Label <input type="text" id="intake-file-tag" placeholder="e.g. Legal Notice Copy / Agreement / FIR" style="margin-bottom:8px;" /></label>
                <button type="button" class="gold-button" id="trigger-intake-file">+ Add Document File</button>
              </div>
            ...
            <div style="display:grid;gap:12px;">
              <label>Choose Service Plan
                <select id="intake-plan-select">
                  <option value="Attorney Shield Consultation|1999" ${intakeData.plan.includes('Attorney Shield') ? 'selected' : ''}>🛡️ Attorney Shield Consultation — ₹1,999 (Full strategy & review)</option>
                  <option value="Emergency Video SOS|1500" ${intakeData.plan.includes('Video') ? 'selected' : ''}>🆘 Emergency Video SOS — ₹1,500 (15 min instant advocate)</option>
                  ...
                  <option value="Chamber Office Visit|799" ${intakeData.plan.includes('Office') ? 'selected' : ''}>🏛️ Chamber Office Visit — ₹799 (20 min in-person)</option>
                  <option value="Doorstep Advocate Visit|2499" ${intakeData.plan.includes('Doorstep') ? 'selected' : ''}>🚗 Doorstep Advocate Visit — ₹2,499 (Advocate comes to you)</option>
                </select>
              </label>
              <div style="background:#ecfdf5;border:1px solid #a7f3d0;border-radius:14px;padding:12px;font-size:0.86rem;color:#065f46;">
                <strong>🔒 Work Completion Hold Active:</strong> Your fee of <strong>₹${intakeData.price}</strong> will be held securely in escrow. Funds are only released after your session is complete and marked <em>Request Entertained</em>.
              </div>
              <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:12px;font-size:0.84rem;">
                ...
                <div>Case Title: ${escapeHtml(intakeData.caseTitle || 'Legal Inquiry')} (${escapeHtml(intakeData.caseType)})</div>
                <div>Court: ${escapeHtml(intakeData.courtName || 'Tis Hazari Court')}</div>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:8px;border-top:1px solid #e2e8f0;padding-top:12px;">
            ...
            ${currentStep < 4 ? `<button type="button" class="gold-button" id="intake-next-step">Next Step →</button>` : `<button type="button" class="gold-button" style="background:#0f766e!important;color:#fff!important;" id="intake-submit-pay">Pay ₹${intakeData.price} & Submit Booking</button>`}
          </div>
        </div>
      `;
  ...
  async function submitIntakeBooking(intakeData) {
    ...
    try {
      ...
      setFlowStatus("Booking Created", `${intakeData.plan} booking #${bookingId} created. Status: 1. Booking Submitted.`);
      activateView("client");
      ...
      try {
        await apiFetch(`/api/bookings/${targetId}/stage`, {
          ...
          body: JSON.stringify({
            ...
            assignedAdvocateId: getSession()?.user?.id || "demo-advocate-id",
          }),
        });
        await loadAndInjectBookings();
      } catch (err) {
  ...
  function adminDashboard() {
    return `
      <div class="lc-role-shell" data-role-dashboard="admin">
        <section class="role-dash-hero">
          <span class="role-dash-kicker">Admin Control Room</span>
          <h2 class="role-dash-title">Platform supervision &amp; 5-Stage Booking Governance.</h2>
          <div class="lc-role-metrics">
            <div class="lc-role-metric"><span>Users</span><strong>Active</strong></div>
            <div class="lc-role-metric"><span>Verifications</span><strong>Pending</strong></div>
            <div class="lc-role-metric"><span>Work Hold Escrow</span><strong>Active</strong></div>
          </div>
        </section>
        <section class="role-dash-card">
          <h3>Live Booking &amp; Request Governance</h3>
  ...
  function clientDashboard() {
    return `
      <div class="lc-role-shell" data-role-dashboard="client">
        <section class="role-dash-hero">
          <span class="role-dash-kicker">Client Portal · Transparent OS</span>
          <h2 class="role-dash-title">Welcome back, ${escapeHtml(displayName())}.</h2>
          <p class="role-dash-sub">Track active bookings, case intake details, and live advocate connections with zero jargon.</p>
          <div class="lc-role-metrics">
            <div class="lc-role-metric"><span>Active Matter</span><strong>Stage 3 of 5</strong></div>
            <div class="lc-role-metric"><span>Escrow Hold</span><strong>Active</strong></div>
            <div class="lc-role-metric"><span>Assigned Lawyer</span><strong>Adv. Rishika Nagpal</strong></div>
          </div>
        </section>

        <section class="role-dash-card">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <h3>Active Booking &amp; Live Status Stepper</h3>
  ...
  function advocateDashboard() {
    return `
      <div class="lc-role-shell" data-role-dashboard="advocate">
        <section class="role-dash-hero">
          <span class="role-dash-kicker">Advocate Command · Practice Cockpit</span>
          <h2 class="role-dash-title">Adv. ${escapeHtml(displayName())}, your client desk is active.</h2>
          <p class="role-dash-sub">Review client intake, case files, CNR numbers, and advance consultation stage status with 1 click.</p>
          <div class="lc-role-metrics">
            <div class="lc-role-metric"><span>Client Bookings</span><strong>Active</strong></div>
            <div class="lc-role-metric"><span>Pending Action</span><strong>1</strong></div>
            <div class="lc-role-metric"><span>Escrow Release</span><strong>On Completion</strong></div>
          </div>
        </section>

        <section class="role-dash-card">
          <h3>Client Bookings &amp; Intake Desk</h3>
          <div data-stage-stepper-container></div>
        </section>
      </div>`;
  }
