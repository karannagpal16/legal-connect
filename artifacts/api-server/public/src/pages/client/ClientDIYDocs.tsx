    ...
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-3xl font-serif font-bold text-[#1A2332]">DIY Legal Documents</h1>
        </div>
      ...
      {!selectedDoc ? (
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-widest text-[#1A2332]/40 mb-4">Choose a Document</h2>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-[#1A2332] font-bold">{t.title}</h3>
                      <span className="text-[10px] text-[#1A2332]/30 bg-[#1A2332]/5 px-2 py-0.5 rounded-full border border-[#1A2332]/10">{t.category}</span>
                    </div>
                    <p className="text-[#1A2332]/50 text-sm">{t.desc}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <button
            ...
            className="flex items-center gap-2 text-[#1A2332]/40 hover:text-[#1A2332] text-sm mb-6 transition-colors"
          >
            ← Back to Documents
          </button>
          ...
          <div className={`border rounded-2xl p-5 mb-6 ${selectedDoc.bg}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-[#1A2332] font-bold text-lg">{selectedDoc.title}</h2>
                <p className="text-[#1A2332]/40 text-xs">{selectedDoc.desc}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            {selectedDoc.fields.map(field => (
              <div key={field.key} className={field.multiline ? "sm:col-span-2" : ""}>
                <label className="block text-[#1A2332]/60 text-xs font-semibold uppercase tracking-wider mb-2">{field.label}</label>
                {field.multiline ? (
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-card/40 border border-[#1A2332]/10 rounded-xl px-4 py-3 text-[#1A2332] text-sm placeholder:text-[#1A2332]/25 focus:outline-none focus:border-blue-500/40 resize-none"
                  />
                ) : (
                  <input
                    value={values[field.key] || ""}
                    onChange={e => setValues(v => ({ ...v, [field.key]: e.target.value }))}
                    placeholder={field.placeholder}
                    className="w-full bg-card/40 border border-[#1A2332]/10 rounded-xl px-4 py-3 text-[#1A2332] text-sm placeholder:text-[#1A2332]/25 focus:outline-none focus:border-blue-500/40"
                  />
                )}
              </div>
            ))}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-[#1A2332] py-4 rounded-xl font-bold text-base transition-all hover:shadow-lg hover:shadow-blue-500/30 mb-6"
          >
          ...
          {generated && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[#1A2332] font-semibold">Your Document is Ready</h3>
                <div className="flex gap-2">
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-1.5 bg-[#1A2332]/10 hover:bg-[#1A2332]/20 text-[#1A2332]/60 hover:text-[#1A2332] px-3 py-2 rounded-xl text-xs font-bold transition-all border border-[#1A2332]/10"
                  >
              ...
              <pre className="bg-card/40 border border-[#1A2332]/10 rounded-2xl p-5 text-[#1A2332]/80 text-xs leading-relaxed whitespace-pre-wrap font-mono max-h-96 overflow-y-auto">
                {generated}
              </pre>
